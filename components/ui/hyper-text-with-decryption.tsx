...
const Word = ({
  ...
  return (
    <motion.span
      className={`
        relative inline-block font-sans font-semibold whitespace-nowrap rounded-md px-2 py-1 -mx-2 -my-1
        ${isHighlightable ? "cursor-pointer" : "cursor-default"}
        ${isHighlightable ? "bg-primary/10" : ""}
      `}
      ...
      animate={{
        ...
        color: isHovered ? "hsl(var(--primary-foreground))" : isHighlightable ? "hsl(var(--primary) / 0.8)" : "hsl(var(--foreground))",
    ...
    >
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.span
            className="absolute -inset-2 rounded-lg bg-primary z-[-1]"
            ...
            style={{
              boxShadow:
                "0px 10px 25px -5px hsl(var(--primary) / 0.4), 0px 8px 10px -6px hsl(var(--foreground) / 0.1)",
            }}
          />
        )}
      </AnimatePresence>
      ...
      {/* The Text Content */}
      <span className="relative z-10 px-1 font-sans">{displayText}</span>
        ...
        {isHovered && (
          <>
            ...
            <motion.span
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0 }}
              className="absolute -bottom-1 -left-1 w-2 h-2 bg-accent rounded-full z-20"
            />
          </>
        )}
      </AnimatePresence>
...
export default function HyperTextParagraph({
  ...
  return (
    <div className={`leading-relaxed tracking-normal ${className}`}>
      {words.map((word, i) => {
